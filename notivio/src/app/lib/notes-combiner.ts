...
function combineTitle(results: ChunkedNoteResult[]): string {
  ...
  const titles = results.map((r) => r.title).filter((t) => !t.includes("Video Notes") && !t.includes("Section"))
 ...
 */
function combineSections(results: ChunkedNoteResult[]): CombinedNotes["sections"] {
    ...
    if (result.sections && Array.isArray(result.sections)) {
      result.sections.forEach((section) => {
        ...
        if (existingIndex !== undefined) {
          ...
          existing.keyInsights = deduplicateArray([...existing.keyInsights, ...(section.keyInsights || [])])
        } else {
          // Add as new section
          const newSection = {
            title: section.title,
  ...
  return allSections.slice(0, 12).map((section) => ({
    ...
    learningObjectives: deduplicateArray(section.learningObjectives).slice(0, 4),
...
function combineSummaries(results: ChunkedNoteResult[]): string {
  ...
  const combinedSummary = summaries
    .map((summary) => summary.replace(/^This (video|section|part)/, "The content"))
    .join(" ")
    .replace(/\s+/g, " ")
    .trim()
 ...
 */
function combineStudyGuides(results: ChunkedNoteResult[]): CombinedNotes["studyGuide"] {
  const combined = {
  ...
  results.forEach((result) => {
    if (result.studyGuide) {
      combined.reviewQuestions.push(...(result.studyGuide.reviewQuestions || []))
      combined.practiceExercises.push(...(result.studyGuide.practiceExercises || []))
  ...
  return {
    ...
    practiceExercises: deduplicateArray(combined.practiceExercises).slice(0, 15),
 ...
 */
function combineConcepts(results: ChunkedNoteResult[]): CombinedNotes["concepts"] {
    ...
    if (result.concepts && Array.isArray(result.concepts)) {
      result.concepts.forEach((concept) => {
        ...
        if (conceptMap.has(key)) {
          ...
          existing.examples = deduplicateArray([...existing.examples, ...(concept.examples || [])])
          existing.relatedTerms = deduplicateArray([...existing.relatedTerms, ...(concept.relatedTerms || [])])

          // Use the longer, more detailed definition
          if (concept.definition && concept.definition.length > existing.definition.length) {
            existing.definition = concept.definition
          }
          if (concept.importance && concept.importance.length > existing.importance.length) {
...
function combineQuizzes(results: ChunkedNoteResult[]): CombinedNotes["quiz"] {
  ...
  const easyQuestions = uniqueQuestions.filter((q) => q.difficulty === "easy").slice(0, 4)
  const mediumQuestions = uniqueQuestions.filter((q) => q.difficulty === "medium").slice(0, 4)
  const hardQuestions = uniqueQuestions.filter((q) => q.difficulty === "hard").slice(0, 2)

  return {
    questions: [...easyQuestions, ...mediumQuestions, ...hardQuestions].slice(0, 10),
  }
}
 ...
 */
function determineDifficulty(results: ChunkedNoteResult[]): "beginner" | "intermediate" | "advanced" {
  // Simple heuristic based on content complexity
  const avgConceptsPerChunk = results.reduce((sum, r) => sum + (r.concepts?.length || 0), 0) / results.length
  const avgSectionsPerChunk = results.reduce((sum, r) => sum + (r.sections?.length || 0), 0) / results.length
